...
import { api, Product, Category, Customer, Transaction as ApiTransaction } from '@/lib/api';
...
export function useStoreApi() {
  ...
  const addCategory = useCallback(async (category: Omit<Category, 'id'>) => {
    try {
      ...
      setCategories(prev => [...prev, convertApiCategory(result)]);
      return convertApiCategory(result);
    } catch (error: any) {
  ...
  const addCustomer = useCallback(async (customer: Omit<Customer, 'id'>) => {
    try {
      ...
      setCustomers(prev => [...prev, convertApiCustomer(result)]);
      return convertApiCustomer(result);
    } catch (error: any) {
  ...
  const getCartTotal = useCallback(() => {
    return cart.reduce((total, item) => total + (getDiscountedPrice(item) * (item.quantity || 1)), 0);
  }, [cart]);

  const getCartTotalDiscount = useCallback(() => {
    return cart.reduce((total, item) => total + (getDiscountAmount(item) * (item.quantity || 1)), 0);
  }, [cart]);
