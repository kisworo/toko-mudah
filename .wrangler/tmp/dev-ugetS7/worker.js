...
// .wrangler/tmp/bundle-52BH6A/checked-fetch.js
...
async function requireSuperAdmin(request, env) {
  ...
  ).bind(authUser.userId, "superadmin").first();
  if (!user) {
    return null;
  }
  ...
  async fetch(request, env, ctx) {
    ...
    try {
      if (path === "/api/auth/register" && request.method === "POST") {
        ...
        const user = { id, username, email, full_name: businessName, is_demo: 0, role: "user", is_active: 1 };
      ...
      if (path.startsWith("/api/admin/users/") && request.method === "PUT" && !path.includes("/toggle-active")) {
        ...
        if (data.username !== void 0) {
          ...
          ).bind(data.username, userId2).first();
          if (usernameExists) {
      ...
      if (path.startsWith("/api/admin/users/") && path.endsWith("/toggle-active") && request.method === "PUT") {
        ...
        ).bind(data.is_active ? 1 : 0, userId2).run();
        const updatedUser = await env.DB.prepare(
          "SELECT id, username, email, full_name, is_demo, role, is_active, created_at, updated_at FROM users WHERE id = ?"
        ).bind(userId2).first();
        return Response.json(updatedUser, { headers: corsHeaders });
...
// .wrangler/tmp/bundle-52BH6A/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-52BH6A/middleware-loader.entry.ts
